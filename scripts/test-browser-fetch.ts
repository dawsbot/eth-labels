...
async function main() {
  ...
  try {
    ...
    const hasLabels =
      html.includes("labelcloud") || html.includes("Label Cloud");
    console.log(`   HTML length: ${html.length}, has labels: ${hasLabels}`);

    if (!hasLabels) {
      console.log(
        "   ❌ Labelcloud fetch failed - got blocked or wrong content",
      );
      ...
      .object({
        d: z
          .object({
            ...
            data: z
              .array(z.object({ contractAddress: z.string().optional() }))
              .optional(),
          })
          .optional(),
      })
      .parse(JSON.parse(tokenResult));
    const tokenCount = parsed.d?.recordsTotal;
    const firstToken = parsed.d?.data?.[0];
    console.log(`   Total tokens: ${tokenCount}`);
    console.log(
      `   First token address field (truncated): ${firstToken?.contractAddress?.substring(0, 80)}...`,
    );
    console.log("   ✅ POST works!\n");

    console.log("4️⃣  Testing account HTML fetch...");
    const accountHtml = await fetcher.fetchHtml(
      "https://etherscan.io/accounts/label/aave?size=100",
    );
    const hasTable =
      accountHtml.includes("table-subcatid") || accountHtml.includes("tbody");
    console.log(
      `   HTML length: ${accountHtml.length}, has table: ${hasTable}`,
    );

    if (!hasTable) {
      console.log("   ⚠️  Trying full navigation instead...");
      const navHtml = await fetcher.navigateAndGetHtml(
        "https://etherscan.io/accounts/label/aave?size=100",
      );
      const hasNavTable =
        navHtml.includes("table-subcatid") || navHtml.includes("tbody");
      console.log(
        `   Nav HTML length: ${navHtml.length}, has table: ${hasNavTable}`,
      );
      console.log(
        hasNavTable ? "   ✅ Navigation fetch works!" : "   ❌ Still no table",
      );
    } else {
